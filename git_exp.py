...
def frag_table(table_number, selected_option='UBL', exp_option=EXP_OPTION):
    ...
    if ('share_list2' in st.session_state) and ('share_list3' in st.session_state):
        ...
        if len(hist_df) > 30:
            repo.update_file(file_path, commit_message, curr, content.sha)
        else:
            curr = pd.concat([hist_df, curr], axis=0).reset_index(drop=True)
            repo.update_file(file_path, commit_message, curr, content.sha)

repo_owner = 'PragatiAgrawal04'
repo_name = 'Live_OC'
file_path = 'history.csv'
token = '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'
